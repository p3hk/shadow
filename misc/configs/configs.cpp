...
#define SAVE_GLOBAL(var) config[#var] = globals::var;
    ...
    SAVE_GLOBAL(chams_opacity)
        ...
        save_file(appdata_path() + "\\charm\\configs\\" + static_cast<std::string>(name) + ".cfg", config.dump());
}

void RBX::configs::load(const char* name)
{
    std::string cfg_json = "";
    load_file(appdata_path() + "\\charm\\configs\\" + static_cast<std::string>(name) + ".cfg", cfg_json);
