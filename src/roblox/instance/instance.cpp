...
namespace RBX {
    ...
    std::string Instance::GetName() const {
        return mem::readString(read<std::uint64_t>(this->address + Offsets::Name));
    }

    std::string Instance::GetClass() const {
        std::uint64_t classAddress = read<std::uint64_t>(this->address + Offsets::Class_Name);
        return mem::readString(read<std::uint64_t>(classAddress + Offsets::Size));
    }
    ...
    std::vector<Instance> Instance::GetChildren(bool isHumanoid) const {
        ...
        for (auto instance = read<std::uint64_t>(start); instance != end; instance += 16u) {
            children.emplace_back(read<Instance>(instance));
        }
        return children;
    }
    ...
    bool Instance::SetProperty(const std::string& propertyName, std::uint64_t value) {
        ...
        write<std::uint64_t>(propertyAddr, value);
        return true;
    }
    ...
    std::uint64_t Instance::GetProperty(const std::string& propertyName) const {
        ...
        return read<std::uint64_t>(propertyAddr);
    }

    bool Instance::Destroy() {
        if (!this->address) return false;
        write<std::uint64_t>(this->address + Offsets::Parent, 0);
        return true;
    }

    bool Instance::Clone(Instance& newInstance) const {
        if (!this->address) return false;
        newInstance = read<Instance>(this->address);
        return newInstance.address != 0;
    }

    bool Instance::MoveTo(Instance& parent) {
        if (!this->address || !parent.address) return false;
        write<std::uint64_t>(this->address + Offsets::Parent, parent.address);
        return true;
    }
    Instance Instance::GetParent() {
        return read<Instance>(this->address + Offsets::Parent);
    }
