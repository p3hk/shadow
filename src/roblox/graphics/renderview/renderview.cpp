...
namespace RBX {
    ...
    std::uint64_t Instance::GetRenderView() {
        return read<uintptr_t>(TaskScheduler{}.GetJobByName("RenderJob") + 0x218);
    }

    std::uint64_t Instance::GetCameraMatrix() {
        auto renderView = GetRenderView();
        return renderView == 0 ? 0 : read<uintptr_t>(renderView + 0x58);
    }
    ...
    std::pair<int, int> Instance::GetViewportSize() {
        ...
        return { read<int>(renderView + 0x2D0), read<int>(renderView + 0x2D0) };
    }
    ...
    void Instance::SetRenderFeature(std::string featureName, bool enable) {
        ...
        uintptr_t featureAddress = read<uintptr_t>(renderView + 0x1DA);
        if (featureName == "Bloom") {
            write<bool>(featureAddress + 0x14C, enable);
        }
        else if (featureName == "Shadows") {
            write<bool>(featureAddress + 0x2A0, enable);
        }
    }
