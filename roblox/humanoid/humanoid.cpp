...
float RBX::Instance::GetHealth() const {
    auto one = read<std::uint64_t>(this->address + Offsets::Health);
    auto two = read<std::uint64_t>(read<std::uint64_t>(this->address + Offsets::Health));
    conv.hex = one ^ two;
    return conv.f;
}

std::string  RBX::Instance::getPlayerDisplayName() {
    uint64_t name = read<uint64_t>(this->address + 0xC8);
   if (!name) {
       return "Nigga";
   }
   else {
       return mem::readString(name);
   }
}
void RBX::Instance::SetHumanoidWalkSpeed(float WalkSpeed)
{
    write<float>(this->address + Offsets::WalkSpeed1, WalkSpeed);
    write<float>(this->address + Offsets::WalkSpeed2, WalkSpeed);
    return;
}
void RBX::Instance::SetHumanoidJumpPower(float JumpPower)
{
    write<float>(this->address + Offsets::JumpPower1, JumpPower);
    return;
}
float RBX::Instance::GetMaxHealth() const {
    auto one = read<std::uint64_t>(this->address + Offsets::Max_Health);
    auto two = read<std::uint64_t>(read<std::uint64_t>(this->address + Offsets::Max_Health));
...
int RBX::Instance::getHumanoidState() const {
    return read<int>(this->address + 0x850);
}
