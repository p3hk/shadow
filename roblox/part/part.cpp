...
namespace RBX {
    void Instance::SetColor(const std::string& part_name, RBX::Vector3 color) {
        ...
        if (primitive) {
            write<RBX::Vector3>(primitive + 0x190, color);
        }
    }
    ...
    void Instance::SetMaterial(const std::string& part_name) {
        ...
        std::uint64_t primitive = part.GetPart();
        if (primitive) {
            write<int16_t>(primitive + 0x2F0, static_cast<int16_t>(utils::enums::material::ForceField));
    ...
    int Instance::GetPartMaterial() const {
        return read<int>(this->address + 0x2F0);
    }

    bool Instance::setCanCollide(bool value) {
        return write<bool>(this->address + Offsets::CanCollide, value);
    }

    RBX::Vector3 Instance::GetPosition() const {
        return read<RBX::Vector3>(GetPart() + Offsets::Part_Position);
    }

    RBX::Vector3 Instance::GetVelocity() const {
        return read<RBX::Vector3>(GetPart() + Offsets::Part_Velocity);
    }

    RBX::Matrix3x3 Instance::GetRotation() const {
        return read<RBX::Matrix3x3>(GetPart() + Offsets::Part_Rotation);
    }

    RBX::Vector3 Instance::GetSize() const {
        return read<RBX::Vector3>(GetPart() + Offsets::Part_Size);
    }
    Vector3 Instance::SetSize(Vector3 size) const {
        return write<Vector3>(GetPart() + Offsets::Part_Size, size);
    }
    RBX::CFrame Instance::GetCframe() {
        RBX::CFrame res{};
        std::uint64_t primitive = GetPart();
        if (primitive) {
            res = read<RBX::CFrame>(primitive + Offsets::CFrame);
        }
        return res;
    }
