...
int_fast64_t RBX::Instance::GetGameId() {
    int_fast64_t game_id = read<int_fast64_t>(this->address + Offsets::GameID);
    return (game_id > 0 && game_id <= 9999999999) ? game_id : 0;
}

RBX::Instance RBX::Instance::GetDataModelPTR() {
    RBX::Instance Instance2;
    return static_cast<RBX::Instance>(read<uintptr_t>(Instance2.GetRenderView() + Offsets::DataModel));
}

RBX::Instance RBX::Instance::GetDataModel() {
    RBX::TaskScheduler Instance;
    RBX::Instance Instance2;
    std::uint64_t datamodel = read<uintptr_t>(Instance2.GetRenderView() + Offsets::DataModel);
    return static_cast<RBX::Instance>(read<std::uint64_t>(datamodel + Offsets::Game));
}


int_fast64_t RBX::Instance::GetCurrentPlaceId() {
    return read<int_fast64_t>(this->address + 0x170);
}
...
std::string RBX::Instance::GetJobId() {
    uint64_t jobIdPtr = read<uint64_t>(this->address + 0x110);
    if (jobIdPtr == 0) return "null id ptr";

    std::vector<char> jobId = read_array<char>(jobIdPtr, 64);
