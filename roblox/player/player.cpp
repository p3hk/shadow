...
namespace RBX {
    ...
    int Instance::getPlayerArmor() {
        for (const auto& player : globals::cached_players) {
            return read<int>(player.armor_obj.address + Offsets::Value);
        }
        return 0;
    }
    ...
    RBX::Instance Instance::GetLocalPlayer() const {
        return read<RBX::Instance>(this->address + Offsets::Local_Entity);
    }

    RBX::Instance Instance::GetModelInstance() const {
        return read<RBX::Instance>(this->address + Offsets::Model_Instance);
    }

    std::int32_t Instance::GetRigType() {
        return read<std::uint8_t>(this->address + Offsets::Humanoid_RigType);
    }

    RBX::Instance Instance::GetModelPrimaryPart() {
        return read<RBX::Instance>(this->address + Offsets::Primary_Part);
    } // returns hrp

    RBX::Instance Instance::GetTeam() const {
        return read<RBX::Instance>(this->address + Offsets::Team);
    }
    ...
    void Instance::SetPartPos(RBX::Vector3 position) {
        ...
        if ((position - cached_position).magnitude() > threshold) {
            auto primitive = read<std::uint64_t>(this->address + Offsets::Part_Primitive);

            for (int i = 0; i < 10000; i++) {
                write<Vector3>(primitive + Offsets::Part_Position, position);
            }
    ...
    void Instance::writePositionMultipleTimes(std::uint64_t primitive, RBX::Vector3 position) {
        ...
        write<RBX::Vector3>(primitive + Offsets::Part_Position, position);
        write<RBX::Vector3>(primitive + Offsets::Part_Position, position);
        write<RBX::Vector3>(primitive + Offsets::Part_Position, position);
        write<RBX::Vector3>(primitive + Offsets::Part_Position, position);
        write<RBX::Vector3>(primitive + Offsets::Part_Position, position);
    ...
    void Instance::SetPartVelocity(RBX::Vector3 velocity) {
        ...
        if ((velocity - cached_velocity).magnitude() > threshold) {
            ...
            for (int i = 0; i < 1000; i++) {
                write<Vector3>(primitive + Offsets::Part_Velocity, velocity);
