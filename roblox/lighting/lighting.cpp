...
void RBX::Instance::setFogEnd(float value) {
    write<float>(this->address + Offsets::Lighting_FogEnd, value);
}

void RBX::Instance::setFogStart(float value) {
    write<float>(this->address + Offsets::Lighting_FogStart, value);
}

void RBX::Instance::setAmbientColor(RBX::Vector3 color) {
    write<RBX::Vector3>(this->address + Offsets::Lighting_AmbientColor, ConvertColor(color));
}

void RBX::Instance::setOutdoorAmbient(RBX::Vector3 color) {
    write<RBX::Vector3>(this->address + Offsets::Lighting_OutdoorAmbient, ConvertColor(color));
}

void RBX::Instance::setBrightness(float value) {
    write<float>(this->address + Offsets::Lighting_Brightness, value);
}

void RBX::Instance::setGlobalShadows(bool enabled) {
    write<bool>(this->address + 0x134, enabled);
}

void RBX::Instance::setBackDoorAmbienceColor(RBX::Vector3 color) {
    write<RBX::Vector3>(this->address + 0xFC, ConvertColor(color));
}

void RBX::Instance::setColorShiftBottom(RBX::Vector3 color) {
    write<RBX::Vector3>(this->address + 0xDC, ConvertColor(color));
}

void RBX::Instance::setColorShiftTop(RBX::Vector3 color) {
    write<RBX::Vector3>(this->address + 0xE8, ConvertColor(color));
}

void RBX::Instance::setFogColor(RBX::Vector3 color) {
    write<RBX::Vector3>(this->address + 0xF0, ConvertFogColor(color));
}

int RBX::Instance::GetColor3() {
    return read<int>(this->address + 0x1770);
}

void RBX::Instance::setColor3(RBX::Vector3 color) {
    write<RBX::Vector3>(this->address + 0x1770, ConvertColor(color));
}
