...
static void run(RBX::PlayerInstance player, RBX::Vector3 resolvedVelocity)
{
    ...
    if (globals::aimbot_type == 0) { // camlock
        ...
        RBX::Instance camera = globals::game.FindFirstChild("Workspace", false, {}).FindFirstChild("Camera", false, {});
        if (!camera.address) {
            RBX::Log_System::Error("Camera not found using spectate method");
            return;
        }

        RBX::Vector3 cameraPos = camera.GetCameraPosition();
        RBX::Log_System::Info("Camera Position: " + std::to_string(cameraPos.x) + ", " +
            std::to_string(cameraPos.y) + ", " + std::to_string(cameraPos.z));

        RBX::Matrix3x3 currentRotation = camera.GetCameraRotation();
        RBX::Log_System::Info("Got camera rotation successfully");

        try {
            RBX::Matrix3x3 hit_matrix = Look_At_To_Matrix(cameraPos, hit_position_3D);
            RBX::Matrix3x3 relative_matrix = Slerp_Matrix3(currentRotation, hit_matrix, smoothness_camera);
            ...
            RBX::Log_System::Info("Setting camera rotation");
            camera.SetCameraRotation(relative_matrix);
            RBX::Log_System::Info("Camera rotation set successfully");
        }
        catch (const std::exception& e) {
            RBX::Log_System::Error(std::string("Exception during camera aimbot: ") + e.what());
        }
        catch (...) {
            RBX::Log_System::Error("Unknown exception occurred during camera aimbot operation");
        }
    }
